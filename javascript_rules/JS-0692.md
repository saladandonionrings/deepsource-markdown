# Disallow to pass multiple arguments to scoped slots
**ID:** `JS-0692` | **Link:** [DeepSource](https://deepsource.com/directory/javascript/issues/JS-0692)

![Minor](https://img.shields.io/badge/severity-minor-yellow)![Anti-pattern](https://img.shields.io/badge/type-anti_pattern-purple)

`this.$scopedSlots` members should not have more than 2 arguments.

* Multiple Arguments create inconsistency.* Cannot omit arguments because multiple arguments should be passed in fixed order

## Bad Practice

```javascript
const children = this.$scopedSlots.default(foo, bar)
const children2 = this.$scopedSlots.default(...foo)
```

## Recommended

```javascript
const children1 = this.$scopedSlots.default()
const children2 = this.$scopedSlots.default(foo)
const children3 = this.$scopedSlots.default({ foo, bar })
```
