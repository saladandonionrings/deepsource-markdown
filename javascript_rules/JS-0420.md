# Prevent usage of string literals in JSX
**ID:** `JS-0420` | **Link:** [DeepSource](https://deepsource.com/directory/javascript/issues/JS-0420)

![Major](https://img.shields.io/badge/severity-major-orange)![Style](https://img.shields.io/badge/type-style-blue)

The issue is raised when the analyzer encounters a string literal inside a react component. We recommend wrapping the string literals in a JSX Container, e.g., `{'Text'}`.

There are a few scenarios where you want to avoid string literals in JSX as follows:

* Enforce consistency of code* Executing JavaScript for multi-purpose use cases e.g. translation system, template literal expression

## Bad Practice

```javascript
var Hello = <div>test</div>;
```

## Recommended

```javascript
var Hello = <div>{'test'}</div>;
```
