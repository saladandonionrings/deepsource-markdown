# Migrate `ember-data` to `@ember-data`
**ID:** `JS-0818` | **Link:** [DeepSource](https://deepsource.com/directory/javascript/issues/JS-0818)

![Minor](https://img.shields.io/badge/severity-minor-yellow)![Bug Risk](https://img.shields.io/badge/type-bug_risk-green)

`ember-data` has been split in multiple packages. For instance, its store is now released in `@ember-data/store` package. These packages have been released starting from `ember-data` version 3.11.

Why we have to migrate?

* Reduce Confusion & Bike Shedding* Improve The TypeScript Experience* Simplify The Mental Model* Provide a Clear Subdivision of Packages

## Bad Practice

```javascript
import Model from 'ember-data/model';
import attr from 'ember-data/attr';
```

```javascript
import DS from 'ember-data';
const { Model } = 'ember-data';
```

## Recommended

```javascript
import Model, { attr } from '@ember-data/model';
```
