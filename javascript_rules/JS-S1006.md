...
**ID:** `JS-S1006` | **Link:** [DeepSource](https://deepsource.com/directory/javascript/issues/JS-S1006)
...
* `maxAge`– It determines the number of sections to expect Certificate Transparency.* `enforce`– if`true`, the user agent should refuse future connections that violate the Certificate Transparency policy. If not set, it defaults to`false`.* `reportUri`– if anything fails, it will report the failure to the URL supplied.
