# Avoid square-bracket notation when accessing properties
**ID:** `JS-0049` | **Link:** [DeepSource](https://deepsource.com/directory/javascript/issues/JS-0049)

![Minor](https://img.shields.io/badge/severity-minor-yellow)![Anti-pattern](https://img.shields.io/badge/type-anti_pattern-purple)

In JavaScript, there are two ways to access the properties of an object:

* dot-notation`(object.property)`( Recommended )* square-bracket notation`(object["property"])`( Bad Practice )
The dot notation is preferred because it is easier to read, less verbose, and works better with aggressive JavaScript minimizers.


## Bad Practice

```javascript
const x = object["property"];
```

## Recommended

```javascript
const x = object.property;
x = object[y];
```
