...
**ID:** `JS-0641` | **Link:** [DeepSource](https://deepsource.com/directory/javascript/issues/JS-0641)
...
* The directive is not owned by a custom element.* The directive is a named slot and is on a custom element directly.* The directive is the default slot, is on a custom element directly, and there are other named slots.* The element which has the directive has another`v-slot`directive.* The element which has the directive has another`v-slot`directive that is distributed to the same slot.* The directive has a dynamic argument which uses the scope properties that the directive defined.* The directive is the default slot, is on a custom element directly, and has no value.* The directive has any modifier.
