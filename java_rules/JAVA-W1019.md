...
**ID:** `JAVA-W1019` | **Lien:** [DeepSource](https://deepsource.com/directory/java/issues/JAVA-W1019)
...
* An instance of `Inner` will contain a reference to an instance of `Outer` . If the `Inner` instance continues to exist after all other references to the `Outer` instance are deleted, the `Outer` instance will still exist because of the reference held by the still-alive `Inner` class.
